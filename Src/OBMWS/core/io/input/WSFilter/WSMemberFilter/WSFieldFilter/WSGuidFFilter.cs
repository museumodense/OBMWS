...
namespace OBMWS
{
    public class WSGuidFFilter : WSFieldFilter
    {
        public WSGuidFFilter(WSTableParam _Field, Expression _member, WSOperation _Operation): base(_Field, _member)
        {
        ...
        public override Expression ToExpression()
        {
            ...
            MethodCallExpression memberToString = Expression.Call(member, WSConstants.toFormattedStringMethod);
            ...
            if (memberToString != null)
            {
                ...
                else
                {
                    ...
                    {
                        List<object> list = ((List<Guid>)Value).Select(d => d.ToString()).OfType<object>().ToList();
                        cExpr = GetExpressionContains<string>(memberToString, list);
                    }
                    else
                    {
                        cExpr = Expression.Constant(((Guid)Value).ToString());
                    }
                }
                if (cExpr != null)
                {
                    #region POSITIV
                    if (operation.Match(OPERATIONS.Equal)) return Expression.Call(memberToString, WSConstants.stringEqualsMethod, cExpr);
                    #endregion

                    #region NEGATIVE
                    else if (operation.Match(OPERATIONS.NotEqual)) return Expression.Call(Expression.Negate(memberToString), WSConstants.stringEqualsMethod, cExpr);
                    #endregion
                }
            }
            return null;
        ...
        public class OPERATIONS
        {
            ...
            public static IEnumerable<WSValueOperation> VALUE_OPERATIONS
            {
                get
                {
                    ...
                    {
                        _VALUE_OPERATIONS = typeof(WSGuidFFilter.OPERATIONS).GetFields().Where(f => f.FieldType.IsAssignableFrom(typeof(WSValueOperation))).Select(f => (WSValueOperation)f.GetValue(null));
            ...
            public static IEnumerable<WSStateOperation> STATE_OPERATIONS
            {
                get
                {
                    ...
                    {
                        _STATE_OPERATIONS = typeof(WSGuidFFilter.OPERATIONS).GetFields().Where(f => f.FieldType.IsAssignableFrom(typeof(WSStateOperation))).Select(f => (WSStateOperation)f.GetValue(null));
        ...
        public override bool Equals(object obj) { return obj != null && this.GetType() == obj.GetType() && this.ToString().Equals(((WSGuidFFilter)obj).ToString()); }
