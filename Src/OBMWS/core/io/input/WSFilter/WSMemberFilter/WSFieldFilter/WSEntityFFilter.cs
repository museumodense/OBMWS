...
namespace OBMWS
{
    public class WSEntityFFilter : WSFieldFilter
    {
        public WSEntityFFilter(WSTableParam _Field, Expression _member, WSOperation _Operation)
            : base(_Field, _member)
        {
        ...
        {
            if (operation.Match(OPERATIONS.Exist) || (operation.Match(OPERATIONS.NotEqual) && Value == null)) { 
                return Expression.NotEqual(member, Expression.Constant(null, Field.DataType)); 
            }
            else if (operation.Match(OPERATIONS.Empty) || (operation.Match(OPERATIONS.Equal) && Value == null)) { 
                return Expression.Equal(member, Expression.Constant(null, Field.DataType));
            }
            else if (operation.Match(OPERATIONS.IsOwn)) {
                //ADD LOGIC HERE...
            }
            else if (Value != null && Value is WSFilter) {
                return ((WSFilter)Value).ToExpression();
            }
            return null;
        }
        ...
        public class OPERATIONS
        {
            ...
            public static readonly WSValueOperation Filter =    new WSValueOperation("Filter",  WSOperation.OperatorChars.Filter,   new List<string> { });
            ...
            public static readonly WSStateOperation IsOwn =     new WSStateOperation(WSConstants.ALIACES.IS_OWN.NAME,WSOperation.OperatorChars.IsOwn,    WSConstants.ALIACES.IS_OWN.ALIACES);
        }

        public override string ToString() { string text = base.ToString(); return text; }
        public override bool Equals(object obj) { return obj != null && this.GetType() == obj.GetType() && this.ToString().Equals(((WSEntityFFilter)obj).ToString()); }
